...
class DepartmentOfSpeechLanguageAndHearingSciencesSpider(scrapy.Spider):
    name = 'department_of_speech_language_and_hearing_sciences_spider'
    start_urls = ['https://aac.sfsu.edu']
    output_folder = r'C:\Users\Fonta\Box\ATI\PDF Accessibility\SF State Website PDF Scans\aac-sfsu-edu'

    @classmethod
    def from_crawler(cls, crawler, *args, **kwargs):
        spider = super(DepartmentOfSpeechLanguageAndHearingSciencesSpider, cls).from_crawler(crawler, *args, **kwargs)
    ...
    def parse(self, response):
        ...
        Extracts all <a href="..."> links from the page and decides:
          - If the link is within aac.sfsu.edu, follow it or store if it's a PDF.
          - If the link is a Box.com link, hand it off to parse_box_link.
        """

        # Regex for aac.sfsu.edu domain (handles both http and https)
        access_url_pattern = re.compile(r'https?://aac.sfsu.edu/.*', re.IGNORECASE)
        ...
        for link in extracted_links:
            ...
            # If the link is internal to aac.sfsu.edu
