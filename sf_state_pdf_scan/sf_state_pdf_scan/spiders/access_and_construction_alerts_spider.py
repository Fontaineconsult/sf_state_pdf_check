...
class AccessAndConstructionAlertsSpider(scrapy.Spider):
    name = 'access_and_construction_alerts_spider'
    start_urls = ['https://accessalert.sfsu.edu']
    output_folder = r'C:\Users\Fonta\Box\ATI\PDF Accessibility\SF State Website PDF Scans\accessalert-sfsu-edu'

    @classmethod
    def from_crawler(cls, crawler, *args, **kwargs):
        spider = super(AccessAndConstructionAlertsSpider, cls).from_crawler(crawler, *args, **kwargs)
    ...
    def parse(self, response):
        ...
        Extracts all <a href="..."> links from the page and decides:
          - If the link is within accessalert.sfsu.edu, follow it or store if it's a PDF.
          - If the link is a Box.com link, hand it off to parse_box_link.
        """

        # Regex for accessalert.sfsu.edu domain (handles both http and https)
        access_url_pattern = re.compile(r'https?://accessalert.sfsu.edu/.*', re.IGNORECASE)
        ...
        for link in extracted_links:
            ...
            # If the link is internal to accessalert.sfsu.edu
