...
class DisabilityProgramsAndResourceCenterSpider(scrapy.Spider):
    name = 'disability_programs_and_resource_center_spider'
    start_urls = ['https://access.sfsu.edu']
    output_folder = r'C:\Users\Fonta\Box\ATI\PDF Accessibility\SF State Website PDF Scans\access-sfsu-edu'

    @classmethod
    def from_crawler(cls, crawler, *args, **kwargs):
        spider = super(DisabilityProgramsAndResourceCenterSpider, cls).from_crawler(crawler, *args, **kwargs)
    ...
    def parse(self, response):
        ...
        Extracts all <a href="..."> links from the page and decides:
          - If the link is within access.sfsu.edu, follow it or store if it's a PDF.
          - If the link is a Box.com link, hand it off to parse_box_link.
        """

        # Regex for access.sfsu.edu domain (handles both http and https)
        access_url_pattern = re.compile(r'https?://access.sfsu.edu/.*', re.IGNORECASE)
        ...
        for link in extracted_links:
            ...
            # If the link is internal to access.sfsu.edu
